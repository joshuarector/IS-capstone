...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($bankName_err) && empty($amount_err) && empty($rate_err) && empty($startDate_err) && empty($maturityDate_err) && empty($term_err) && empty($clientName_err) && empty($brokerName_err)){
        ...
        $sql = "UPDATE transactions SET bank_name=?, amount=?, rate=?, startDate=?, maturityDate=?, term=?, client_name=?, broker_name=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssi", $param_bankname, $param_amount, $param_rate, $param_startdate, $param_maturitydate, $param_term, $param_clientname, $param_brokername, $param_id);
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        <div class="form-group <?php echo (!empty($bankName_err)) ? 'has-error' : ''; ?>">
